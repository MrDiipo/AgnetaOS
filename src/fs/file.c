...
int fopen(const char *filename, const char *mode_string) {
    ...
    // wew cannot just have a root path
    if (!root_path->first) {
    ...
    void* descriptor_private_data = disk->filesystem->open(disk, root_path->first, mode); // calls fat16 open
    ...
    out:
        if (res < 0) res = 0; // fopen should not return negative values
        return res;
}
