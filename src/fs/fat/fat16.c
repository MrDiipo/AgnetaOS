...
#define AGNETAOS_FAT16_SIGNATURE 0x29
#define AGNETAOS_FAT16_FAT_ENTRY_SIZE 0x02
#define AGNETAOS_FAT16_BAD_SECTOR 0xFF7
#define AGNETAOS_FAT_16_UNUSED 0x00
...
struct fat_private {
    ...
    // used t stream the file allocation table
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector) {
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != AGNETAOS_ALL_OK) {
        goto out;
    }
    while (1) {
        if (diskstreamer_read(stream, &item, sizeof(item)) != AGNETAOS_ALL_OK) {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory) {
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != AGNETAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    if (diskstreamer_read(stream, dir, root_dir_size) != AGNETAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    directory->item = dir;
...
int fat16_resolve(struct disk *disk) {
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != AGNETAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != AGNETAOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    out:
...
void fat16_get_full_relative_filename(struct fat_directory_item *item, char *out, int max_len) {
    ...
    if (item->ext[0] != 0x00 && item->ext[0] != 0x20) {
        // there must have been an extension
        *out_tmp++ = '.';
...
static int fat16_get_fat_entry(struct disk *disk, int cluster) {
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * AGNETAOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0) goto out;
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset) {
    ...
    for (int i = 0; i < clusters_ahead; i++) {
        ...
        if (entry == AGNETAOS_FAT16_BAD_SECTOR) {
            res = -EIO;
            goto out;
        }
        // Reserved sector
...
fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, int cluster, int offset, int total,
    ...
    if (res != AGNETAOS_ALL_OK) goto out;
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name) {
    ...
    char tmp_filename[AGNETAOS_MAX_PATH];
    // file metadata is already loaded into memory
...
int fat16_seek(void* private, uint32_t offset, FILE_SEEK_MODE seek_mode) {
    ...
    switch (seek_mode) {
        case SEEK_SET: desc->pos = offset;
            break;
        case SEEK_END: res = -EUNIMP;
            break;
        case SEEK_CUR: desc->pos += offset;
            break;
        default: res = -EINVARG;
            break;
    }
